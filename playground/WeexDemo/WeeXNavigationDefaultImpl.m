...
@implementation WeeXNavigationDefaultImpl
#pragma mark -
#pragma mark WeeXNavigationProtocol
...
//    [NSURL URLWithString:param[@"url"] ];
//    NSURL *url =param[@"url"];
//    NSMutableDictionary *wxoptions=[NSMutableDictionary new];
//    wxoptions =param[@"options"];
    WeeXBaseViewController *vc = [[WeeXBaseViewController alloc]initWithSourceURL:[NSURL URLWithString:param[@"url"]] wxoptions:param[@"options"]];
    vc.hidesBottomBarWhenPushed = YES;
//     vc.wxoptions = param[@"options"];
    [container.navigationController pushViewController:vc animated:animated];
    [self callback:block code:MSG_SUCCESS data:nil];
}

- (void) taogubapushViewControllerWithParam:(NSMutableDictionary *)param completion:(WXNavigationResultBlock)block
                      withContainer:(UIViewController *)container
{
    if (0 == [param count] || !param[@"url"] || !container) {
        [self callback:block code:MSG_PARAM_ERR data:nil];
