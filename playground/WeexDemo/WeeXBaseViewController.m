...
@property (nonatomic, strong) NSDictionary *wxoptions;
@end
...
- (void)_renderWithURL:(NSURL *)sourceURL
{
    ...
    if (!self.wxoptions) {
        self.wxoptions=[NSDictionary new];
    }
    //NSDictionary *noptions=[NSDictionary new];
    [self.wxoptions setValue:sourceURL.absoluteString forKey:@"bundleUrl"];
    [self.wxoptions setValue:@"0" forKey:@"skinType"];
     [_instance renderWithURL:[NSURL URLWithString:newURL] options:self.wxoptions data:nil];
